...
[[Embed]]s are arbitrary client-side apps authored in a Workspace and rendered on desktop. Because an Embed is arbitrary authored code, it must be treated as **untrusted code that auto-executes when a Markdown File is opened**. We render each Embed in a **sandboxed iframe** (unique opaque origin, no `allow-same-origin`), so its code cannot reach the host realm — the Electron preload bridge, the app origin, or `localStorage`. All Workspace/filesystem access happens only through a host-controlled postMessage broker.
...
- All Embed data access is **async** (brokered RPC), not direct calls — this fixes the query-hook API shape before any Embed is written.
