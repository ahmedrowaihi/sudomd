...
Opening a large folder in Sudomd Desktop should keep the editor usable and avoid recursive filesystem watchers. The sidebar is a refreshable Markdown File snapshot, while the currently open file is watched directly for external edits.
...
1. When a user opens a Workspace Folder or Plain Folder, Sudomd Desktop loads the folder and builds a sidebar snapshot from markdown files.
...
7. Ignored folders do not appear in the sidebar and do not need to be opened by Sudomd to find sidebar-visible children.
...
9. Sudomd does not maintain a recursive filesystem watcher for the sidebar file tree.
10. When the app window regains focus, Sudomd refreshes the sidebar snapshot for the current workspace.
...
Use Sudomd Desktop with a large repo folder:
...
6. Create, rename, or delete a markdown file outside the app; focus Sudomd or choose `Sync Workspace` and confirm the sidebar refreshes.
