...
const TEXT_EXT = new Set([
	...
	".css", ".html", ".md", ".yaml", ".yml", ".astro", ".txt",
]);
...
function loadConfig(args) {
	...
	return {
		...
		theme: {
			...
			brandAccentForeground: args.brandAccentForeground ?? theme.brandAccentForeground,
		},
		dry: !!args.dry,
	};
}
...
async function main() {
	...
	log(cfg, "");
	log(cfg, cfg.dry ? "Dry run complete — no files changed." : "Rebrand complete.");
	if (!cfg.dry) {
...
function buildRules(from, to, cur, cfg) {
	...
	return rules.filter(([a, b]) => a && a !== b).sort((x, y) => y[0].length - x[0].length);
}
...
function patchDesktopPkg(cfg, cur) {
	...
	if (cfg.repo) {
		pkg.repository = { ...pkg.repository, url: `git+https://github.com/${cfg.repo}.git` };
...
function patchReleaseWorkflow(cfg) {
	...
	const next = fs.readFileSync(file, "utf8").split(oldPrefix).join(cfg.tagPrefix);
	if (!cfg.dry) fs.writeFileSync(file, next);
...
async function generateIcon(cfg) {
	...
	if (cfg.dry) {
		log(cfg, `  icon  would write assets/icon.png from ${cfg.icon} (electron-builder converts it)`);
		return;
	}
	const sharp = await tryImport("sharp");
	if (sharp) {
		await roundSquircle(sharp, src, out);
		log(cfg, "  icon  wrote rounded assets/icon.png (macOS squircle via sharp)");
	} else {
...
async function roundSquircle(sharp, src, out) {
	const size = 1024, art = 824, inset = 100, radius = 185;
