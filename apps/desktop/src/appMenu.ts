...
async function createFileSubmenu(actions: { open: () => void }): Promise<Submenu> {
  return Submenu.new({
    text: "File",
    items: [
      await MenuItem.new({ id: "open", text: "Open…", accelerator: "CmdOrCtrl+O", action: () => actions.open() }),
      await PredefinedMenuItem.new({ text: "separator-text", item: "Separator" }),
...
export async function createAppMenu(actions: { open: () => void }): Promise<Menu> {
  const items: Submenu[] = [await createFileSubmenu(actions), await createEditSubmenu()];
