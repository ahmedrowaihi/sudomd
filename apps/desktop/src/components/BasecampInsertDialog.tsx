...
async function runBasecampSearch(
	query: string,
): Promise<{ items: BasecampSearchItem[]; error: string | null }> {
	try {
		const result = await desktopApi.searchBasecamp(query);
		if (result.ok) return { items: result.items, error: null };
...
type ThreadMarkdownOutcome =
	| { ok: true; markdown: string; commentCount: number }
	| { ok: false; message: string; description?: string };

async function fetchThreadMarkdown(
	url: string,
): Promise<ThreadMarkdownOutcome> {
	try {
		const result = await desktopApi.fetchBasecamp(url);
		if (!result.ok || !result.html) {
			return {
				ok: false,
				message: "Couldn't fetch from Basecamp",
				description: result.error,
			};
		}
		return {
			ok: true,
			markdown: buildThreadMarkdown(result),
			commentCount: result.comments?.length ?? 0,
		};
	} catch {
		return { ok: false, message: "Failed to insert from Basecamp" };
	}
}
...
export function BasecampInsertDialog({
	...
	useEffect(() => {
		...
		const timer = window.setTimeout(async () => {
			const outcome = await runBasecampSearch(trimmed);
	...
	async function insert(url: string, label?: string) {
		if (!url) return;
		setInsertingUrl(url);
		const outcome = await fetchThreadMarkdown(url);
		setInsertingUrl(null);
		if (!outcome.ok) {
			toast.error(outcome.message, { description: outcome.description });
			return;
		}
		window.dispatchEvent(
			new CustomEvent(INSERT_MARKDOWN_EVENT, {
				detail: { markdown: outcome.markdown },
			}),
		);
		const count = outcome.commentCount;
